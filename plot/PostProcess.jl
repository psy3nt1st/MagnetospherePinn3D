function create_test(n_q, n_μ, n_ϕ, t1, NN, Θ, st, params; use_θ = false)
	...
	dBr_dq, dBθ_dq, dBϕ_dq, dα_dq, dBr_dμ, dBθ_dμ, dBϕ_dμ, dα_dμ, dBr_dϕ, dBθ_dϕ, dBϕ_dϕ, dα_dϕ  = calculate_derivatives(q, μ, ϕ, t, Θ, st, NN, params)[1:12]
    ...
    # Nα_S = reshape(Nα_S, n_μ, n_ϕ)

	return q, μ, ϕ, t, Br1, Bθ1, Bϕ1, α1, ∇B, B∇α, Nr, Nθ, Nϕ, Nα, Nα_S
end
...
function integrand(x, p)
    ...
    Nr, Nθ, Nϕ, Nα, Nα_S = evaluate_subnetworks(q, μ, ϕ, 0, Θ, st, NN)
...
function field_line_equations!(du, u, p, t)
    ...
    Nr, Nθ, Nϕ, Nα, Nα_S = evaluate_subnetworks(q, μ, ϕ, t1, Θ, st, NN)


	Br1 = Br(q, μ, ϕ, Θ, st, Nr[1], params)
	Bθ1 = Bθ(q, μ, ϕ, Θ, st, Nθ[1])
	Bϕ1 = Bϕ(q, μ, ϕ, Θ, st, Nϕ[1])
	B = @. √(Br1^2 + Bθ1^2 + Bϕ1^2)

    # println("$q, $μ, $ϕ, $t1, $Br1, $Bθ1, $Bϕ1, $B, $Nr, $Nθ, $Nϕ, $Nα, $Nα_S")
